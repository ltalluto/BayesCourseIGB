...
optim(0.5, lik_func, n=dat$died, k=dat$n, method="Brent", lower=0, upper=1)

par(mar=c(4.5,4.5,0.2,0.2))
plot(seq(0,1,0.01), sapply(seq(0,1,0.01), lik_func, n=dat$died, k=dat$n), type='l', col='#0066cc', bty='n', xlab=expression(theta), ylab="Negative Log Likelihood", lwd=3.5)
...
nll_count1 <- function(lambda, n) {
	-sum(dpois(n, lambda, log=TRUE))
}

nll_count2 <- function(musize, n) {
	-sum(dnbinom(n, mu=musize[1], size=musize[2], log=TRUE))
}

res1 <- optim(5, nll_count1, n=dat$n, lower=0, upper=max(dat$n), method='Brent')
res2 <- optim(c(5, 5), nll_count2, n=dat$n)
